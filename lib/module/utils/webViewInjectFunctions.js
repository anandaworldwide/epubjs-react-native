...
export function mapAnnotationStylesToEpubStyles(type, styles) {
  let epubStyles = {};
  if (type === 'highlight') {
    epubStyles = {
...
export function mapArrayObjectsToAnnotations(array) {
  return `
...
export function onChangeAnnotations(annotations = 'Object.values(rendition.annotations._annotations)') {
  return `
...
export function addAnnotation(type, cfiRange, data, iconClass, styles, cfiRangeText, noEmit = false) {
...
export function addAnnotationByTagId(type, tagId, data, iconClass, styles, cfiRangeText, noEmit = false) {
  const epubStyles = mapAnnotationStylesToEpubStyles(type, styles);
  if (type === 'mark') {
    // eslint-disable-next-line no-param-reassign
    iconClass = iconClass || 'epubjs-mk-balloon';
...
export function updateAnnotation(annotation, data = {}, styles) {
...
export function updateAnnotationByTagId(tagId, data = {}, styles) {
  return `
...
export function removeAnnotationByTagId(tagId) {
  return `
    async function getCfiByTagId(tagId) {
      const results = await Promise.all(book.spine.spineItems.map((item) => {
        return item.load(book.load.bind(book)).then(() => {
