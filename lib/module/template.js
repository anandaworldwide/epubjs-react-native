    ...
    <script>
      ...
      function touchStartHandler(e) {
        if (e.touches && e.touches.length === 1) {
          ...
          debugLog('Touch start at x=' + e.touches[0].clientX);
        }
      }
      ...
      // Attach touch handlers to a document for tap detection
      function attachTouchHandlers(doc) {
        if (!doc || documentsWithClickHandler.has(doc)) return;
        documentsWithClickHandler.add(doc);
        debugLog('Attaching touch handlers to document: ' + (doc.title || 'untitled'));
        ...
        if (views && views.length) {
          views.forEach(function(view) {
            if (view && view.document) {
              attachTouchHandlers(view.document);
            }
          });
        }
      }
      ...
      // Use content hook to attach touch handlers when each content document is loaded
